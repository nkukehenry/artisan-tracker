        ...
        <>
            ...
            <div className={`relative ${className}`}>
                <button
                    ...
                    className="flex items-center justify-between w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:border-gray-400 dark:hover:border-gray-500 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent transition-colors"
                >
                    <div className="flex items-center gap-3">
                        <Smartphone className="h-5 w-5 text-gray-500 dark:text-gray-400" />
                        <div className="text-left">
                            {selectedDevice ? (
                                <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{selectedDevice.name}</div>
                            ) : (
                                <div className="text-sm text-gray-500 dark:text-gray-400">Select a device</div>
                            )}
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        {selectedDevice && (
                            <div
                                ...
                                className="p-1 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-full transition-colors cursor-pointer"
                                role="button"
                                tabIndex={0}
                                onKeyDown={(e) => {
                ...
                <div className="fixed inset-0 bg-black/50 dark:bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full mx-4 border border-gray-200 dark:border-gray-700">
                        ...
                        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Select Device</h2>
                            <div className="flex items-center gap-2">
                                <button
                                    onClick={handleRefreshDevices}
                                    disabled={isLoading}
                                    className="p-1 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        {device.owner && (
                                                            <div className="text-xs text-gray-400 dark:text-gray-500">Owner: {device.owner}</div>
                                                        )}
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-3">
