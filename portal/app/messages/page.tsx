  ...
  const columns = [
    {
      ...
      render: (item: Message, value: unknown) => (
        <span className={`inline-flex items-center gap-2 px-3 py-3 rounded-full text-xs font-medium ${(value as string) === 'SMS' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300' :
          (value as string) === 'WHATSAPP' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' :
            (value as string) === 'TELEGRAM' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300' :
    ...
    },
    {
      ...
      sortable: true,
      render: (item: Message, value: unknown) => (
        <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${value ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300' : 'bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-300'}`}>
    ...
    },
    {
      ...
      render: (item: Message) => (
        <button
          ...
          className="flex items-center gap-1 px-3 py-1 text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-md transition-colors"
        >
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">View message history for {selectedDevice.name}</p>
            </div>
          </div>
              ...
              {isFiltering && (
                <div className="absolute inset-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm z-10 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">Filtering messages...</p>
                  </div>
                </div>
              )}
              <DataTable
